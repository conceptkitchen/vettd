...
exports.learnFromFindings = learnFromFindings;
exports.getLearnedPatterns = getLearnedPatterns;
exports.getLearnedPatternsCount = getLearnedPatternsCount;
exports.getLearnedPatternsForScanner = getLearnedPatternsForScanner;
...
async function ensureCache() {
    if (cacheLoaded)
        return;
    try {
        const persisted = await (0, db_js_1.getPersistedPatterns)();
...
async function generatePattern(finding) {
...
async function learnFromFindings(deepFindings) {
    await ensureCache();
    const newPatterns = [];
    for (const finding of deepFindings) {
        // Don't learn duplicates
        const isDuplicate = learnedPatterns.some((lp) => lp.learnedFrom === finding.evidence.slice(0, 200));
        if (isDuplicate)
            continue;
        ...
        if (pattern) {
            ...
            // Persist to database
            await (0, db_js_1.saveLearnedPattern)({
...
async function getLearnedPatternsCount() {
    // Try DB first for accurate count
    try {
        const dbCount = await (0, db_js_1.getPersistedPatternCount)();
        if (dbCount > 0)
            return dbCount;
    }
    catch {
 ...
 */
async function getLearnedPatternsForScanner() {
    await ensureCache();
